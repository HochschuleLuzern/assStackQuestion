...
class assStackQuestionDB
{
	...
	public static function _readInputs($question_id, bool $just_id = false): array
	{
		...
		while ($row = $db->fetchAssoc($res)) {
            ...
            // Bug in CAS causes wrong notes and feedback - so following (check_answer_type) effectively ignored
	...
	public static function _saveUserTestSolution(assStackQuestion $question, int $active_id, int $pass, bool $authorized): int
	{
        ...
        if (array_key_exists('prts', $question->getEvaluation())) {
            foreach ($question->getEvaluation()['prts'] as $prt_name => $prt) {
                ...
                if (isset($question->getEvaluation()['points'][$prt_name]['prt_points'])) {
                    self::_addPointsToPRTDBEntry($question, $active_id, $pass, $prt_name,
                        $question->getEvaluation()['points'][$prt_name]['prt_points'], $authorized);
                }
                ...
                $question->saveCurrentSolution($active_id, $pass, 'xqcas_prt_' . $prt_name . '_status',
                    (string) $fraction);
                $entered_values++;

                //value1 = xqcas_input_*_status_message, $value2 = answernotes
                $question->saveCurrentSolution($active_id, $pass, 'xqcas_prt_' . $prt_name . '_answernote',
                    implode(';', $prt->_answernotes));
                $entered_values++;
